...
<span style="color: red;">Red font is meant to show missing areas as reminders for me to improve on.</span>
...
Maybe force the IV to be [,1] and the DV to be [,2] so the following steps can just refer to those columns? Otherwise generalize it so that following tests call upon the placeholder value.

Though this wouldn't work as for paired data, specific columns would be selected to compare between.</span>
...
In order to check the assumptions of the paired t-test, the following tests can be conducted:
bla
(2) bla 
...
In hypothesis testing, the interest is in ascertaining whether or not the null hypothesis of no effect is tenable, that is, whether or not the null hypothesis can be retained as a reasonable account of the observed data. Specifically, the hypothesis test will assess the extent to which the observed data are surprising if the null hypothesis were true. The test’s main output is a p-value: the probability of encountering a test statistic that is at least as extreme as the one that is observed, given that the null hypothesis is true.

You conducted a `r ttest$method`.

#### Parameter Estimation

In parameter estimation, the interest is in quantifying the size of the effect. Specifically, the estimation procedure assesses the set of parameter values that would not be rejected by a hypothesis test of level $\alpha$. This set forms a confidence interval. For instance, the set of parameter values that would not be rejected with a test of $\alpha = 5%$ form a 95% confidence interval. In other words, the confidence interval contains the values that are not statistically incompatible with the observed data. 
...
if (is_paired == TRUE) {
paired_ttest <- t.test(data = data_paired, Moon~Other, var.equal = TRUE, alternative = alternative, paired = is_paired)
welch_ttest <- t.test(data = data_paired, extra~group, var.equal = FALSE, alternative = alternative)
paired_wilcox <- wilcox.test(data = data_paired, extra~group, alternative = alternative, paired = is_paired)
...
#  welch_twoside_ttest <- t.test(data = data_paired, extra~group, var.equal = FALSE, alternative = "two.sided")
#  welch_less_ttest <- t.test(data = data_paired, extra~group, var.equal = FALSE, alternative = "less")
#  welch_more_ttest <- t.test(data = data_paired, extra~group, var.equal = FALSE, alternative = "greater")
