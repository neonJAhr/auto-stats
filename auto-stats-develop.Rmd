...
☑ Student's Sleep Data & Paired Samples t-Test

Student's Sleep Data `sleep` is selected. 

```{r data_input, include=FALSE}
dataKitchen <- read.csv("kitchenRollsTTest.csv")
data <- sleep

# TO BE FIXED: data_paired was originally assigned sleep, but it is NOT paired data!
data_paired <- read.csv("moonAggression.csv")
# data_paired <- sleep
```

The IV is `group`, and the DV is `extra`. 
...
VISUAL CONFIRMATION OF EQUAL VARIANCE BETWEEN DATA SETS & OUTLIERS
...
QUESTION: Should we only use Brown-Forsythe test? "The B-F test is less likely than the Levene test to incorrectly declare that the assumption of equal variances has been violated." (https://www.statisticshowto.com/brown-forsythe-test/) `r colorize('INSERT/TODO: If quoted, this needs to go into references!', "red")`

```{r LV_homogeneity_of_variance, include = FALSE}
# Find Levene's test as code
leveneTest(data$extra, group = data$group)
levTest <- leveneTest(data$extra, group = data$group)
...
dixon <- dixon.test(data[,1])
grubbs <- grubbs.test(data[,1])
...
if (sw_test$p.value < 0.05) {
  ...
  sw_test_result <- "The Shapiro-Wilk test showed that the data does not differ significantly from normal distribution. (That is good!)"
}
```

The Shapiro-Wilk test is a statistical test that tests the null hypothesis that a sample came from a normally distributed population. `r sw_test_result`
...
ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = TRUE, alternative = "two.sided")

if (outlier_test == TRUE & levTest$`Pr(>F)`[1] <= 0.05 & sw_test$p.value >= 0.05) {
  # ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = TRUE, alternative = "two.sided")
  ttest <- t.test(data = data, extra~group, var.equal = TRUE, paired = TRUE, alternative = "less")
...
# Brown-Forsythe
bfTest <- onewaytests::bf.test(extra ~ group, data = data)
...
"Although paired t test are relatively robust to skewness, you could either do a transformation (e.g. log-transform), or consider to compare the results to a rank-based test"
...
if (abs(skew) > 1) {
  ...
  skewOutput <- cat("The distribution of your data is moderately skewed, which means you cannot assume a normality in your distribution. This is not necessarily a problem, as long as your data set should be sufficiently large (n per group < 20) to have a valid t-test result. Alternatively, you run could a different test, e.g. the Mann-Whitney U test or Kruskal-Wallis test.")
} else {skewOutput <- "distribution of your data is only mildly skewed, which means that you can assume normality for your data sets."}
```
Last, a skewness test was conducted to see how non-symmetric your data set is. `r skewOutput`
...
INSERT/TODO: If the user does a one-way or one-sample t-test, include the following for normality assumptions: One common misconception when doing hypothesis testing with an ANOVA is that the normality assumption must be met for all groups. However, the one-way ANOVA is considered a robust test against the normality assumption, meaning that it tolerates violations to its normality assumption rather well1.
...
INSERT/TODO Still needs updating to only compare either independent or paired tests.
For a discussion on choosing between the t-test and nonparametric alternatives, see Lumley, et al. (2002).
...
ind_twoside_ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = TRUE, alternative = "two.sided")
ind_less_ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = TRUE, alternative = "less")
ind_more_ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = TRUE, alternative = "greater")

paired_twoside_ttest <- t.test(data = data_paired, Moon~Other, var.equal = TRUE, alternative = "two.sided", paired = TRUE)
paired_less_ttest <- t.test(data = data_paired, Moon~Other, var.equal = TRUE, alternative = "less", paired = TRUE)
paired_more_ttest <- t.test(data = data_paired, Moon~Other, var.equal = TRUE, alternative = "greater", paired = TRUE)

welch_twoside_ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = FALSE, alternative = "two.sided")
welch_less_ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = FALSE, alternative = "less")
welch_more_ttest <- t.test(data = dataKitchen, mean_NEO~Rotation, var.equal = FALSE, alternative = "greater")

ind_twoside_wilcox <- wilcox.test(data = dataKitchen, mean_NEO~Rotation, alternative = "two.sided")
ind_less_wilcox <- wilcox.test(data = dataKitchen, mean_NEO~Rotation, alternative = "less")
ind_more_wilcox <- wilcox.test(data = dataKitchen, mean_NEO~Rotation, alternative = "greater")

paired_twoside_wilcox <- wilcox.test(data = data_paired, Moon~Other, alternative = "two.sided", paired = TRUE)
paired_less_wilcox <- wilcox.test(data = data_paired, Moon~Other, alternative = "less", paired = TRUE)
paired_more_wilcox <- wilcox.test(data = data_paired, Moon~Other, alternative = "greater", paired = TRUE)
```

<span style="color: red;">Have the user choose whether the alternative is two sided, less, or greater, for purpose of demonstration it's less. 

INSERT/TODO: TABLE DOESN'T WORK YET</span>

<span style="color: red;">Fix Actual tests for moonAgression dataset.</span>
...
One common misconception about Hypothesis Testing is that after computing an appropriate decision statistic under NHST (Null Hypothesis Significance Testing), you determined whether or not this provides sufficient evidence to reject the assumption that the Null Hypothesis is true. However, this does not mean that we have proven which hypothesis is true. If there are multiple hypotheses as to why there could be a difference between the populations, consider doing an ANOVA, where you can test between more than two hypotheses. 
...
#### Misconceptions about Parameter Estimation
